...
# r'D:\A-Fkf\MVSNet_pytorch-master\trying'
parser.add_argument('--filepath', default=r'D:\A-Fkf\MVSNet_pytorch-master\new_data\out24\dense')
...
parser.add_argument('--outdir', default=r'D:\FKF\aiFrac\pytorch-mvsnet-master\outputs', help='output dir')
...
def save_depth():
    ...
    test_dataset = MVSDataset(args.filepath, "test", 5, args.numdepth, args.interval_scale)
...
def filter_depth(scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        h, w, _ = ref_img.shape
        '''
        h_knife, w_knife = knife()

        color = ref_img[1:h - h_knife:4, 1:w - w_knife:4, :][valid_points]  # hardcoded for DTU dataset
        '''
        color = ref_img[1: h: 4, 1: w: 4, :][valid_points]
...
def knife():
    ...
    for image_path in os.listdir(args.filepath + "/images"):
        h, w, _ = cv2.imread("{}/{}".format(args.filepath + "/images", image_path)).shape
        height = min(height, h)
        weight = min(weight, w)
    return height % 32, weight % 32
...
if __name__ == '__main__':
    ...
    for scan in scans:
        ...
        filter_depth(scan_folder, out_folder, os.path.join(args.outdir, '{}_mvsnet_{}.ply'.format(args.test_name, scan.split('\\')[-2])))
