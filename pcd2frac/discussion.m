...
% %% Import data
% 
% [FileName,PathName,~] = uigetfile({'*.pcd';'*.xls';'*.xlsx';'*.txt';'*.dat'},'Select point cloud data');
...
%     figure;
%     pcshow(ql_pcData(:,1:3),ql_pcData(:,10),"BackgroundColor",[0 0 0]) 
%     view(0,30);
...
while a==1
    ...
    [color,J]=find_similar_color(rest_pcData,r2); %J is a set of data selected this time
    % pause;
    answer = 'Yes';
    switch answer
        case 'Yes'  
...
for ii=1:1:n
    ...
    % pcshow(j(:,1:3),[cx(a,:),cy(a,:),cz(a,:)],"BackgroundColor",[1 1 1])  % x, y, z, color
    % grid on;
    % set(gca,'fontname','Times New Roman','fontsize',14);
    % xlabel(gca,'X (mm)','fontname','Times New Roman','fontsize',16 );
    % ylabel(gca,'Y (mm)','fontname','Times New Roman','fontsize',16 );
...
% PathName = uigetdir('C:\', 'select folder to save');

PathName = sprintf('C:\\Users\\a\\Desktop\\discussion\\c%d', r2);
...
for ii=1:1:n
    ...
    xyz=j(:,1:3);
    normals=j(:,4:6);
    colors=[cx(a,:),cy(a,:),cz(a,:)];

    pcdCloud = pointCloud(xyz, color=colors+zeros(size(xyz, 1, size(xyz, 2))), Normal=normals);
    pcwrite(pcdCloud, fullfile(root, num2str(a)),PLYFormat="binary");
